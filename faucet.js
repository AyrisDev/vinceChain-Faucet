...
app.get("/config.json", async (req, res) => {
  ...
  for (let i = 0; i < conf.blockchains.length; i++) {
    ...
    sample[chainConf.name] = "vce1rz40fwp3l3ul6842087zk5e75r9x46fmtrpjyq";

    const wallet2 = Wallet.fromMnemonic(
      process.env.MNO_KEY,
      pathToString(chainConf.sender.option.hdPaths[0])
    );
    console.log(
      "address:",
      "vce1rz40fwp3l3ul6842087zk5e75r9x46fmtrpjyq",
      wallet2.address
    );
  }
  ...
  try {
    ...
    if (chainConf) {
      if (chainConf.type === "Ethermint") {
        ...
        const wallet = Wallet.fromMnemonic(process.env.MNO_KEY).connect(
          ethProvider
        );
        await wallet
          .getBalance()
